...
public class ConvertToFJTaskRefactoring extends Refactoring {
	...
	private void createComputeMethod(TypeDeclaration recursiveActionSubtype, final AST ast, RefactoringStatus result) {
		...
		final Map<Integer, Statement> tasksToStatement= new HashMap<Integer, Statement>();
		...
		fMethodDeclaration.accept(new MethodVisitor(allTaskDeclStatements, tasksToStatement, statementsToTasks, allTaskDeclFlags, locationOfNewBlocks, scratchRewriter, numTasksPerBlock,
		...
		try {
			...
			while (blockIter.hasNext()) {
				...
				atLeastOneBlockChanged= doBlockWork(ast, result, editGroup, scratchRewriter, allTaskDeclStatements, tasksToStatement, statementsToTasks,
	...
	private boolean doBlockWork(final AST ast, RefactoringStatus result, final TextEditGroup editGroup, final ASTRewrite scratchRewriter,
			...
			Map<Statement, List<Integer>> statementsToTasks, Map<Integer, Integer> allTaskDeclFlags, final Map<Block, List<Statement>> allStatementsWithRecursiveMethodInvocation, final Map<Statement, List<String>> allPartialComputationsNames,
...
//			}
	...
	private void createPartialComputations(AST ast, final TextEditGroup editGroup, final ASTRewrite scratchRewriter, final List<String> partialComputationsNames,
			final List<String> typesOfComputations, ListRewrite listRewriteForBlock, Statement lastStatementWithRecursiveCall, boolean isNotNewBlock, List<Integer> taskList, int flags) {
		if (lastStatementWithRecursiveCall instanceof VariableDeclarationStatement) {
			if (flags == 3) {  //TODO may need to worry about some recursive calls being together but others that are not that will be left behind - need to also do some sort of linear thing
				VariableDeclarationFragment varFragment= ((VariableDeclarationFragment)(ASTNode.copySubtree(ast, ((VariableDeclarationFragment)(((VariableDeclarationStatement) lastStatementWithRecursiveCall).fragments().get(0))))));
				...
				if (isNotNewBlock) {
					listRewriteForBlock.insertAfter(assignToResult, lastStatementWithRecursiveCall, editGroup);
				} else {
	...
	private final class MethodVisitor extends ASTVisitor {
		...
		private final Map<Integer, Statement> fTasksToStatement;
		...
		private MethodVisitor(Map<Integer, VariableDeclarationStatement> allTaskDeclStatements, Map<Integer, Statement> tasksToStatement, Map<Statement, List<Integer>> statementsToTasks, Map<Integer, Integer> allTaskDeclFlags,
		...
		private void populateAllMaps(List<String> partialComputationsNames, List<String> typesOfComputations, boolean infixExpressionFlag, boolean methodInvocationFlag, Block myBlock,
			...
			fTasksToStatement.put(taskNum, parentOfMethodCall);
