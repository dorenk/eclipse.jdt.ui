...
public class ConvertToFJTaskRefactoring extends Refactoring {
	...
	private boolean attemptRefactoringOnBlock(final AST ast, final TextEditGroup editGroup, final ASTRewrite scratchRewriter, Map<Integer, VariableDeclarationStatement> taskNumberToTaskDeclStatement,
		...
		if (hasParallelRecursiveMethods) {
			...
			if (isNewBlock) {
				scratchRewriter.replace(newlyCreatedBlockToReplacementLocation.get(currBlock), currBlock, editGroup);
			}
			...
			List<Statement> recursiveList= blockToRecursiveInvocations.get(currBlock);
			...
			renameRecursiveCallsToSequentialInNonRefactoredBlock(ast, editGroup, scratchRewriter, newlyCreatedBlockToReplacementLocation, currBlock, isNewBlock);
		}
	...
	private void refactorStatement(final AST ast, final TextEditGroup editGroup, final ASTRewrite scratchRewriter, ListRewrite listRewriteForBlock, Statement currStatement,
		...
		final int[] taskNum= {0};
		currStatement.accept(new ConvertMethodCallToTask(taskList, ast, taskNum, scratchRewriter, editGroup));
		if (!isNotNewBlock) {
			listRewriteForBlock.insertLast(currStatement, editGroup);
		}
	}
	...
	private void renameRecursiveCallsToSequentialInNonRefactoredBlock(final AST ast, final TextEditGroup editGroup, final ASTRewrite scratchRewriter, final Map<Block, Statement> newlyCreatedBlockToReplacementLocation,
		...
		if (!isNewBlock) {
			...
			Statement newStatement= newlyCreatedBlockToReplacementLocation.get(currBlock);
	...
	private boolean hasNoRecursiveCall(RefactoringStatus result, final Map<Block, List<Statement>> blockToRecursiveInvocations) {
		
		if (blockToRecursiveInvocations.size() == 0) {
	...
	private final class FindRecursiveCallsVisitor extends ASTVisitor {
		...
		private void populateAllStatementsWithRecursiveMethodInvocation(Block myBlock, Statement parentOfMethodCall) {
			...
			if (fBlockToRecursiveInvocations.containsKey(myBlock)) {
				List<Statement> recursiveList= fBlockToRecursiveInvocations.get(myBlock);
				...
				recursiveList.add(parentOfMethodCall);
				fBlockToRecursiveInvocations.put(myBlock, recursiveList);
			}
		}
		...
		private Block getNewBlock(Statement targetStatement) {
			...
			if (fLocationOfNewBlocks.containsKey(targetStatement)) {
				...
				fNewlyCreatedBlockToReplacementLocation.put(myBlock, targetStatement);
			}
			return myBlock;
		}
